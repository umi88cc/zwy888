...
function updateUserInterface() {
    ...
    if (userStr && userBtn) {
        try {
            ...
            // 如果有头像 URL，替换掉默认的 font-awesome 图标
            if (user.avatar) {
                userBtn.innerHTML = `<img src="${user.avatar}" style="width:32px; height:32px; border-radius:50%; border:2px solid #fff; box-shadow:0 2px 5px rgba(0,0,0,0.1); vertical-align:middle;">`;
            }
        } catch (e) {
...
function switchTab(tabName) {
    ...
    const tabs = document.querySelectorAll('.tab-header span');
    if(tabName === 'login') tabs[0].classList.add('active'); else tabs[1].classList.add('active');
    loadCaptcha(tabName);
}
...
async function handleRegister(e) { 
    ...
    try {
        ...
        if(d.success) {
            ...
            alert(d.message); closeModal('authModal'); 
            location.reload(); // 刷新后头像会自动变成QQ头像
        } else {
            alert(d.message); loadCaptcha('register'); 
        }
...
function showUserCenter(user) {
    ...
    const adminBtnHtml = (user.role === 'admin') ? `<a href="/admin.html" class="submit-btn" style="display:block;text-align:center;background:#333;margin-top:10px;text-decoration:none;">进入后台管理</a>` : '';
    card.innerHTML = `<button class="close-btn" onclick="closeModal('authModal')">×</button><div style="text-align:center;"><img src="${user.avatar}" style="width:80px;height:80px;border-radius:50%;margin-bottom:10px;border:3px solid #eee;"><h3>${user.username}</h3><p style="color:#888;margin-bottom:15px;">${user.vip_level>0?'<span style="color:#ff6b6b;font-weight:bold">VIP会员</span>':'普通用户'}</p><button onclick="logout()" style="width:100%;padding:10px;border:1px solid #ddd;background:none;border-radius:12px;cursor:pointer;">退出登录</button>${adminBtnHtml}</div>`;
...
async function initPostPage() {
    ...
    if (token) {
        ...
        try {
            const res = await fetch(`${API_BASE}/posts/content/${postId}`, {headers: { 'Authorization': `Bearer ${token}` }});
            const data = await res.json();
            if (data.success) document.getElementById('postContent').innerHTML = data.content;
        } catch(e) {}
    }
}
async function loadComments(postId) { /* 略，复用 */ }
async function checkLikeStatus(postId) { /* 略，复用 */ }
window.submitComment = async function(postId) { /* 略，复用 */ };
window.toggleLike = async function(postId) { /* 略，复用 */ };
