...
// ==========================================
// ⚡️ 数据库一键安装接口 (临时使用)
// 访问地址: https://你的域名/install_umi88_db
// ==========================================
app.get('/install_umi88_db', async (c) => {
    ...
    try {
        ...
        return c.text('✅ 数据库初始化成功！所有表已重置。\n请立即去首页注册一个新账号，它将自动成为管理员。');
    } catch (e) {
...
app.get('/:category/:id.html', async (c) => {
  ...
  return renderPage(c, 'post.html', {
      title: post.title, date: new Date(post.created_at * 1000).toISOString().slice(0,10),
      price: post.price || 0, content: content, id: post.id, like_count: post.like_count || 0
  });
});
...
app.get('/api/posts/list', async (c) => {
  const page = parseInt(c.req.query('page') || '1'); const limit = parseInt(c.req.query('limit') || '10'); const offset = (page - 1) * limit;
  ...
  const posts = results.map(p => { const plainText = p.content ? p.content.replace(/<[^>]+>/g, '').substring(0, 100) + '...' : ''; return { ...p, excerpt: plainText, content: undefined }; });
