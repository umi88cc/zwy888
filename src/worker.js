...
// 部署后，请用浏览器访问: https://umi88.cc/install_umi88_db
// ============================================================
app.get('/install_umi88_db', async (c) => {
    // 这里是你提供的最新数据库结构 (已去除默认管理员插入，保留表结构)
    const sql = `
    -- 1. 用户表
    ...
    CREATE TABLE posts (
        ...
        like_count INTEGER DEFAULT 0,     -- 新增字段
        comment_count INTEGER DEFAULT 0,  -- 新增字段
        created_at INTEGER DEFAULT (strftime('%s', 'now'))
    );
    ...
    try {
        ...
        return c.html(`
            <h1 style="color:green">✅ 数据库初始化成功！</h1>
            <p>所有表已创建/重置。</p>
            <p>请点击 <a href="/">回到首页</a>，然后<b>注册第一个账号</b>，系统会自动将其设为管理员。</p>
        `);
    } catch (e) {
